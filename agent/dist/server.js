    ...
    switch (msg.type) {
        case 'agent:start':
            if (!agent && activeConfig) {
                agent = new clawdbot_1.ClawdBot(activeConfig);
                hookAgentEvents();
                await agent.start();
        ...
        case 'post:queue':
            if (agent) {
                agent.addToQueue(msg.data);
                broadcast('post:queued', msg.data);
            }
            break;
        case 'personality:update':
            if (activeConfig) {
                ...
                if (agent)
                    agent.updatePersonality(msg.data);
                broadcast('personality:updated', msg.data);
            }
            break;
        case 'skill:toggle':
            if (agent) {
                agent.toggleSkill(msg.data.skillId, msg.data.enabled);
                broadcast('skill:toggled', msg.data);
            }
            break;
        case 'skill:register':
            if (agent) {
                agent.registerCustomSkill(msg.data);
                broadcast('skill:registered', msg.data);
            }
            break;
        case 'token:add':
            if (activeConfig) {
                ...
                if (agent)
                    await agent.addToken(msg.data.mint);
        ...
        case 'token:remove':
            if (activeConfig) {
                ...
                if (agent)
                    agent.removeToken(msg.data.mint);
        ...
        case 'account:remove':
            ...
            if (activeAccountId === msg.data.accountId)
                await switchAccount(accounts[0]?.id || null);
        ...
        case 'config:update':
            if (activeConfig) {
                if (msg.data.schedule)
                    Object.assign(activeConfig.schedule, msg.data.schedule);
                if (msg.data.moltBot)
                    Object.assign(activeConfig.moltBot, msg.data.moltBot);
...
function hookAgentEvents() {
    ...
    events.forEach(evt => agent.on(evt, (data) => broadcast(`agent:${evt}`, data)));
    setInterval(() => {
        if (agent)
            broadcast('agent:state', { ...agent.getState(), performance: agent.getPerformance() });
    }, 10_000);
}
// ── ACCOUNT MANAGEMENT ───────────────────────────────────────
function createAccount(data) {
    return {
        ...
        config: {
            ...
            identity: { name: data.name, handle: data.handle, personality: config_1.DEFAULT_CONFIG.identity.personality, avatar: data.avatar || '' },
            ...
            schedule: { postsPerHour: 3, replyDelayMs: 30000, engagementWindowHours: 2, quietHoursUTC: [4, 8] },
        },
        isActive: false,
    };
}
