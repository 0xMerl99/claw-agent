...
exports.MoltBotEvolver = void 0;
class MoltBotEvolver {
    config;
    strategies = new Map();
    generation = 0;
    evolutionLog = [];
    ...
    // ── STRATEGY SELECTION ─────────────────────────────────────
    selectStrategy(context, activeStrategies, performance) {
    ...
    // ── EVOLUTION CYCLE ────────────────────────────────────────
    async evolve(currentPerformance) {
        this.generation++;
        ...
        for (const strategy of strategies) {
            ...
            if (recentHistory.length === 0)
                continue;
            const avgEngagement = recentHistory.reduce((sum, h) => sum + h.metrics.engagementRate, 0) / recentHistory.length;
        ...
        const underperformers = strategies.filter((s) => s.fitness < this.config.performanceThreshold * avgFitness);
        ...
        const topPerformer = strategies.reduce((best, s) => s.fitness > best.fitness ? s : best);
    ...
    updateMetrics(strategyName, executionIndex, metrics) {
        const strategy = this.strategies.get(strategyName);
...
exports.MoltBotEvolver = MoltBotEvolver;
//# sourceMappingURL=moltbot.js.map